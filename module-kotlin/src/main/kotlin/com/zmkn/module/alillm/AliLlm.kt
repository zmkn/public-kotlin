...
import com.zmkn.module.alillm.extension.toMap
import com.zmkn.module.alillm.extension.toResponseMessage
import com.zmkn.module.alillm.extension.toToolCallBase
...
class AliLlm(
    ...
    private fun createGenerationParam(
        ...
        val messages: List<Message> = options.messages.map { message ->
            when (message.role) {
                com.zmkn.module.alillm.enumeration.MessageRole.SYSTEM -> createSystemMessage(message.content)
                com.zmkn.module.alillm.enumeration.MessageRole.USER -> createUserMessage(message.content)
                com.zmkn.module.alillm.enumeration.MessageRole.ASSISTANT -> createAssistantMessage(message.content, message.toolCalls)
                com.zmkn.module.alillm.enumeration.MessageRole.TOOL -> createToolMessage(message.toolCallId!!, message.content)
            }
        }
    ...
    private fun createMultiModalConversationParam(
        ...
        val messages: List<MultiModalMessage> = options.messages.map { message ->
            when (message.role) {
                com.zmkn.module.alillm.enumeration.MessageRole.SYSTEM -> createMultiModalSystemMessage(message.contents)
                com.zmkn.module.alillm.enumeration.MessageRole.USER -> createMultiModalUserMessage(message.contents)
                com.zmkn.module.alillm.enumeration.MessageRole.ASSISTANT -> createMultiModalAssistantMessage(message.contents)
                com.zmkn.module.alillm.enumeration.MessageRole.TOOL -> throw IllegalArgumentException("MessageRole.TOOL is not allowed.")
            }
        }
    ...
    companion object {
        ...
        private fun createFunctionDefinition(options: GenerationParamOptions.Tool): FunctionDefinition = FunctionDefinition
            ...
            .parameters(JsonUtils.parseString(options.schema).getAsJsonObject())
            .build()
